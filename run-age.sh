#!/bin/bash
# Chạy age-script.jsx trong Photoshop mà không cần click dialog — dành cho AI agent / CI.
#
#   ./run-age.sh                    # dùng ./age-config.json
#   ./run-age.sh my-config.json     # dùng config khác
#
...
CONFIG="${1:-$SCRIPT_DIR/age-config.json}"
LOG="$SCRIPT_DIR/age-run.log"
DONE="$SCRIPT_DIR/age-run.done"
...
if [[ ! -f "$CONFIG" ]]; then
  ...
  echo "Copy age-config.example.json thành age-config.json rồi sửa lại." >&2
  exit 1
fi
...
if grep -q '\[NAS\]' "$CONFIG"; then
  ...
  RESOLVED="$SCRIPT_DIR/.age-config-resolved.json"
...
# AGE_CONFIG chỉ là dự phòng khi Photoshop được khởi động mới từ chính shell này.
export AGE_CONFIG="$CONFIG"

MAX_RETRIES="${AGE_MAX_RETRIES:-2}"
attempt=0
while :; do
  if [[ -f "$SCRIPT_DIR/../.cancel-flag" ]]; then
...
# Script ghi OK / ERROR vào age-run.done khi kết thúc
