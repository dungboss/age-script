...
REM Chay age-script.jsx trong Photoshop khong can click dialog - danh cho AI agent / CLI.
REM
REM   run-age.bat                     dung .\age-config.json
REM   run-age.bat my-config.json      dung config khac
REM
REM Photoshop.exe -r khong truyen duoc tham so, nen duong dan config duoc ghi vao
REM age-config-path.txt de script .jsx doc. Photoshop van mo sau khi chay xong,
REM nen batch cho bang cach poll file age-run.done.

set "SCRIPT_DIR=%~dp0"
set "SCRIPT_DIR=%SCRIPT_DIR:~0,-1%"
set "JSX=%SCRIPT_DIR%\age-script.jsx"
set "LOG=%SCRIPT_DIR%\age-run.log"
set "DONE=%SCRIPT_DIR%\age-run.done"
set "POINTER=%SCRIPT_DIR%\age-config-path.txt"

REM Timeout mac dinh 60 phut (612 anh chay rat lau). Doi qua bang bien AGE_TIMEOUT_SEC.
if "%AGE_TIMEOUT_SEC%"=="" set "AGE_TIMEOUT_SEC=3600"

if "%~1"=="" (
  set "CONFIG=%SCRIPT_DIR%\age-config.json"
) else (
...
if not exist "%CONFIG%" (
  ...
  echo Copy age-config.example.json thanh age-config.json roi sua lai. 1>&2
  exit /b 1
)
...
if not errorlevel 1 (
  ...
  set "RESOLVED=%SCRIPT_DIR%\.age-config-resolved.json"
...
if not defined PS_EXE (
  ...
  echo Neu cai o cho khac, dat bien AGE_PS_EXE tro toi Photoshop.exe. 1>&2
  if defined AGE_PS_EXE set "PS_EXE=%AGE_PS_EXE%"
)
if defined AGE_PS_EXE set "PS_EXE=%AGE_PS_EXE%"
...
REM Tu dong thu lai loi Photoshop/timeout. AGE_MAX_RETRIES=2 = tong 3 lan thu.
if "%AGE_MAX_RETRIES%"=="" set "AGE_MAX_RETRIES=2"
set /a MAX_ATTEMPTS=AGE_MAX_RETRIES+1
set /a ATTEMPT=0
set "FINAL_EXIT=3"
...
if !WAITED! geq %AGE_TIMEOUT_SEC% (
  echo Qua thoi gian cho %AGE_TIMEOUT_SEC% giay ma script chua bao xong. 1>&2
